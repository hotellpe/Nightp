@php
	...
	if(!$keywords){
    	if(isset($queryParams) && $queryParams ){
    	    $keywords = (isset($queryParams['q'])) ? $queryParams['q'] : null;
    	}
	    $keywords = (isset($keywords)) && (is_string($keywords)) ? $keywords : null;
	    $keywords = rawurldecode($keywords);
	}
...
<div class="container mb-2 serp-search-bar">
	<form id="search" name="search" action="{{ \App\Helpers\UrlGen::searchWithoutQuery() }}" method="POST">
	    @csrf
		@if (!empty($qFilterBy))
