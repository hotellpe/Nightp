...
@section('after_scripts')
	...
	<script>
		...
		@if (!empty($user->photo) && !empty($user->photo_url))
			@php
				try {
			...
			@endphp
			options.initialPreview[0] = '{{ $user->photo_url }}';
			options.initialPreviewConfig[0] = {};
			options.initialPreviewConfig[0].key = {{ (int)$user->id }};
			options.initialPreviewConfig[0].caption = '{{ basename($user->photo) }}';
